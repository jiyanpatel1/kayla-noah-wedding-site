...
function writeCsv(rsvps) {
  const headers = [
    'submittedAt', 'invitationId', 'invitationName', 'contactName', 'email', 'phone',
    'attendanceStatus', 'guestName', 'guestAttending', 'firstCourse', 'mainCourse', 'dietaryRestrictions', 'songRequest', 'message'
  ];
  const rows = [headers.join(',')];
  for (const rsvp of rsvps) {
    const responses = Array.isArray(rsvp.responses) && rsvp.responses.length ? rsvp.responses : [{}];
...
app.post('/api/find-guest', (req, res) => {
  ...
  const matches = guests.filter((party) => {
    const partyName = normalize(party.displayName);
    const individualNames = (party.guests || []).map(normalize);
    return partyName.includes(query) || individualNames.some((name) => name.includes(query));
  }).slice(0, 8);
...
app.post('/api/rsvp', (req, res) => {
  ...
  if (!invitation) {
    return res.status(404).json({ error: 'This invitation was not found on the guest list.' });
  }
  ...
  })).filter((response) => allowedNames.has(normalize(response.name)) || invitation.guests.length === 0);

  if (attendanceStatus === 'accepts' && cleanResponses.length === 0) {
    return res.status(400).json({ error: 'Please select at least one attending guest.' });
  }

  if (attendanceStatus === 'accepts' && cleanResponses.some((response) => !response.firstCourse || !response.mainCourse)) {
    return res.status(400).json({ error: 'Please choose a first course and main course for each attending guest.' });
  }
  ...
  const submission = {
    ...
    attendanceStatus,
    responses: attendanceStatus === 'declines' ? (invitation.guests || []).map((name) => ({ name, attending: false, firstCourse: '', mainCourse: '' })) : cleanResponses,
    dietaryRestrictions: String(body.dietaryRestrictions || '').trim(),
    songRequest: String(body.songRequest || '').trim(),
    message: String(body.message || '').trim()
  };

  const existingIndex = rsvps.findIndex((r) => r.invitationId === invitationId);
  if (existingIndex >= 0) {
  ...
  writeCsv(rsvps);
  res.json({ ok: true, message: 'RSVP saved.', submission });
});
...
app.listen(PORT, () => {
  ensureFiles();
  console.log(`Kayla & Noah wedding site running at http://localhost:${PORT}`);
